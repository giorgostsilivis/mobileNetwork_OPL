...
//gia kathe cell theloume enan typo anavathmisis
forall(i in cells)
 	upgrade_con:
...
forall(n in areas)
	...
	sum(i in cells, j in type) (k[n][i][j] * (trunc(coef[j]*qualities[i][n]+qualities[i][n]))) >= 8;

//prepei na mhn upervenetai h xwiritikothta tou kathe cell. Dhladh na to athroisma twn xwritikothtwn
//perioxwn pou syndeontai me ena cell prepei na einai mikrothero apo thn xwritikothta tou cell
forall(i in cells)
    capacity_con:
